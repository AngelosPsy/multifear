# Vendored code from the ez package
#
# The functions ezANOVA_aov, ezANOVA_main,
# ezANOVA_between_summary, ezANOVA_get_wide_lm,
# ezANOVA_summary,  ezANOVA_levene, and ezANOVA
...
ezANOVA_main <-	function(
  ...
  if (!is.factor(data[, names(data) == wid])) {
    warning(
      paste('Converting "', wid, '" to factor for ANOVA.', sep = ''),
      immediate. = TRUE,
      call. = FALSE
    )
    data[, names(data) == wid] = factor(data[, names(data) == wid])
  } else{
    if (length(unique(data[, names(data) == wid])) != length(levels(data[, names(data) ==
                                                                         wid]))) {
      warning(
        paste(
          'You have removed one or more Ss from the analysis. Refactoring "',
          wid,
          '" for ANOVA.',
          sep = ''
        ),
  ...
  if (!is.null(within)) {
    for (i in 1:length(within)) {
      if (is.numeric(data[, names(data) == within[i]])) {
        ...
        if (length(unique(data[, names(data) == within[i]])) != length(levels(data[, names(data) ==
                                                                                   within[i]]))) {
          warning(
            paste(
              'You have removed one or more levels from variable "',
              within[i],
  ...
  if (!is.null(between)) {
    for (i in 1:length(between)) {
      if (is.numeric(data[, names(data) == between[i]])) {
        ...
        if (length(unique(data[, names(data) == between[i]])) != length(levels(data[, names(data) ==
                                                                                    between[i]]))) {
          warning(
            paste(
              'You have removed one or more levels from variable "',
    ...
    if (any(wid_temp$Freq > 1)) {
      ...
      for (i in unique(as.character(between))) {
        data[, names(data) == wid] = paste(data[, names(data) == wid], data[, names(data) ==
                                                                              i])
      }
      data[, names(data) == wid] = factor(data[, names(data) == wid])
      temp = ddply(idata.frame(data)
    ...
    balanced = ifelse(length(unique(temp$N)) > 1
                      , FALSE
                      , TRUE)
    if (!balanced) {
      warning(
  ...
  if (!is.null(diff)) {
    ...
    if (!is.factor(data[, names(data) == diff])) {
      warning(
        paste(
          'Converting "',
          diff,
          '" to factor for computing difference score.',
          sep = ''
        ),
        immediate. = TRUE,
        call. = FALSE
    ...
    if (!all(as.data.frame(table(temp[, names(temp) %in% c(wid, within)]))$Freq ==
             2)) {
      stop(
        paste(
          'Variable supplied to "diff" ("',
          as.character(diff),
    ...
    data[, names(data) == as.character(diff)] = factor(data[, names(data) ==
                                                              as.character(diff)])
    ...
    if (reverse_diff) {
      data[, names(data) == as.character(diff)] = factor(data[, names(data) ==
                                                                as.character(diff)], levels = rev(levels(data[, names(data) == as.character(diff)])))
    }
  }
  ########
  ...
  if (!is.null(between_covariates)) {
    ...
    temp <- ddply(temp
                  , structure(as.list(
                    c(wid, between_covariates, within, within_full, diff)
                  ), class = 'quoted')
                  , function(x) {
    ...
    for (cov in between_covariates) {
      ...
      if (is.factor(temp$ezCov)) {
        ...
        warning(
          'Covariate"',
          cov,
          '" is numeric and will therefore be fit to a linear effect.',
          immediate. = TRUE,
          call. = FALSE
        )
      }
      fit = eval(parse(text = paste(
        'lm(formula=', dv, '~ezCov,data=temp)'
      )))
      temp$fitted = fitted(fit)
      for (cov_lev in as.character(unique(temp[, names(temp) == cov]))) {
        data[as.character(data[, names(data) == cov]) == cov_lev, names(data) ==
               dv] = data[as.character(data[, names(data) == cov]) == cov_lev, names(data) ==
                            dv] - temp$fitted[as.character(temp[, names(temp) == cov]) == cov_lev][1] + as.numeric(coef(fit)[1])
      }
    }
  }
  if (!is.null(within_covariates)) {
    ...
    temp <- ddply(temp
                  , structure(as.list(
                    c(wid, within_covariates, within, within_full, diff)
                  ), class = 'quoted')
                  , function(x) {
                    to_return = mean(x$ezDV)
                    names(to_return) = as.character(dv)
    ...
    for (cov in as.character(within_covariates)) {
      temp2 <- ddply(
        .data = temp
        ,
        .variables = eval(parse(
          text = paste('.(', wid, ',', cov, ')')
        ))#structure(as.list(c(wid,as.symbol(cov)))),class = 'quoted')
        ,
        .fun = function(x) {
          to_return = data.frame(value = mean(x[, names(x) == dv]))
          names(to_return) = as.character(dv)
          return(to_return)
        }
      )
      ...
      for (this_wid in unique(as.character(data[, names(data) == wid]))) {
        ...
        fit = eval(parse(
          text = paste('lm(formula=', dv, '~ezCov,data=temp3)')
        ))
        temp3$fitted = fitted(fit)
        for (cov_lev in unique(as.character(temp3[, names(temp3) == cov]))) {
          data[(as.character(data[, names(data) == cov]) == cov_lev) &
                 (data[, names(data) == wid] == this_wid), names(data) == dv] = data[(as.character(data[, names(data) ==
                                                                                                          cov]) == cov_lev) &
                                                                                       (data[, names(data) == wid] == this_wid), names(data) == dv] - temp3$fitted[as.character(temp3[, names(temp3) ==
  ...
  if (!is.null(within_full)) {
    ...
    data <- ddply(temp
                  , structure(as.list(
                    c(wid, between, within, within_full, diff)
                  ), class = 'quoted')
                  , function(x) {
                    to_return = mean(x$ezDV)
                    names(to_return) = as.character(dv)
    ...
    data <- ddply(data
                  ...
                  , function(x) {
                    to_return = data.frame(temp = mean(x[, names(x) == as.character(dv)])
                                           , ezNum = nrow(x))
  ...
  if (is.null(diff)) {
    if (!all(as.data.frame(table(data[, names(data) %in% c(wid, within)]))$Freq ==
             1)) {
      stop('One or more cells is missing data. Try using ezDesign() to check your data.')
    }
  } else{
    if (!all(as.data.frame(table(data[, names(data) %in% c(wid, within, diff)]))$Freq ==
             1)) {
      stop('One or more cells is missing data. Try using ezDesign() to check your data.')
    }
  }
  ...
  if (is.null(between)) {
    if (any(within_numeric)) {
      ...
      #there are no numeric within-Ss variables
      if (type != 1) {
        wide_lm = ezANOVA_get_wide_lm(data, dv, wid, within, between)
        to_return = NULL
        try(to_return <- ezANOVA_summary(Anova(
          wide_lm$lm
          ,
          idata = wide_lm$idata
          ,
    ...
    #there are between-Ss variables
    if (balanced) {
      if (is.null(within)) {
        wide_lm = ezANOVA_get_wide_lm(data, dv, wid, within, between)
        from_Anova = Anova(wide_lm$lm
                           ,
                           type = type
                           ,
                           white.adjust = white.adjust)
        ...
        if (!any(between_numeric)) {
          ...
          warning(
            'At least one numeric between-Ss variable detected, therefore no assumption test will be returned.',
            immediate. = TRUE,
            call. = FALSE
          )
        }
          ...
          if (type != 1) {
            ...
            try(to_return <- ezANOVA_summary(
              Anova(
                ...
                ,
                idesign = eval(
                  parse(text = wide_lm$idesign_formula)
                )
              )
            ))
            if (is.null(to_return)) {
      ...
      if (type == 1) {
        if ((length(between) > 1) | (!is.null(within))) {
          warning(
            'Using "type==1" is highly questionable when data are unbalanced and there is more than one variable. Hopefully you are doing this for demonstration purposes only!',
            immediate. = TRUE,
            call. = FALSE
          )
        }
        ...
        if (is.null(within)) {
          ...
          if (any(within_numeric)) {
            stop(
              'Cannot perform ANOVA when data are imbalanced and when one or more within-Ss variables are numeric. Try ezMixed() instead.'
            )
          }
          wide_lm = ezANOVA_get_wide_lm(data, dv, wid, within, between)
          to_return = NULL
  ...
  to_return$ANOVA = to_return$ANOVA[order(stringr::str_count(to_return$ANOVA$Effect, ':')), ]
  to_return$data = data
  return(to_return)
}

ezANOVA_get_wide_lm <- function(data, dv, wid, within, between) {
  to_return = list()
  if(!is.null(within)) {
...
ezANOVA_aov <- function(data, dv, wid, within, between){
  aov_formula = paste(
    as.character(dv)
    ,'~'
...
####################################################################################################
# Copied from https://github.com/mike-lawrence/ez/blob/master/R/ez-internal.R
ezANOVA_levene <-
  function (y) {
    form <- y
    mf <- model.frame(form)
