...
#'
#' @description Basic function for running frequentist's regression models
...
#'
#'
#' lm_mf(cs1 = "CSP1", cs2 = "CSM1", subj = "id", data = example_data)
#'
#' @importFrom stats lm
#' @export
...
lm_mf <-
  function(cs1,
    ...
    # You need to take different scores
    data <-
    ...
    # You need to take different scores
    if (is.null(group)) {
      lm_res <- lm(cs.2 - cs.1 ~ 1, data = data)
    } else {
      lm_res <- lm(cs.2 - cs.1 ~ group_2, data = data)
    }
