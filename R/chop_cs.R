...
chop_cs <- function(cs, data, subj, na.rm = FALSE) {
  ...
  if ((nc_cs %% 2) == 0) {
    # Half of the trials
    cs_fhalf <-
      data %>% dplyr::select(1, all_of(nc_cs) / 2) %>%
      rowMeans(na.rm = na.rm) %>%
      tibble::enframe(name = NULL, value = "cs_fhalf")
    cs_lhalf <-
      data %>% dplyr::select(all_of(nc_cs) / 2 + 1, all_of(nc_cs)) %>%
      rowMeans(na.rm = na.rm) %>%
    ...
    # more of that
    cs_fhalf <-
      data %>% dplyr::select(1, (all_of(nc_cs) + 1) / 2) %>%
      rowMeans(na.rm = na.rm) %>%
      tibble::enframe(name = NULL, value = "cs_fhalf")
    cs_lhalf <-
      data %>% dplyr::select((all_of(nc_cs) + 1) / 2 + 1, all_of(nc_cs)) %>%
      rowMeans(na.rm = na.rm) %>%
      tibble::enframe(name = NULL, value = "cs_lhalf")
  }
