...
chop_css <- function(cs1, cs2, data, subj, cs_paired = NULL, na.rm = FALSE) {
  # Check data
  collection_warning(
    cs1 = cs1,
    cs2 = cs2,
  ...
  cs1_tmp <- multifear::chop_cs(cs = cs1, data = data, subj = subj, prefix = "cs1")
  cs2_tmp <- multifear::chop_cs(cs = cs2, data = data, subj = subj, prefix = "cs2")

  if (!is.null(cs_paired)){
    cs2_paired <- multifear::chop_cs(cs = cs2[1:length(cs_paired)], data = data, subj = subj, prefix = "cs2_p")
    cs_paired <- multifear::chop_cs(cs = cs_paired, data = data, subj = subj, prefix = "cs1_p")
  }

  # Compute differential scores. Maybe delete?
  csbind <- dplyr::bind_rows(cs2_tmp %>% tibble::rownames_to_column(),
  ...
  if(!is.null(cs_paired)){
    ...
    csbind_2 <- dplyr::bind_rows(cs2_paired %>% tibble::rownames_to_column(),
